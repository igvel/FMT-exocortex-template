...
LOG_DIR="/home/igvel/logs/synchronizer"
STRATEGY_DIR="/home/igvel/repos/IWE/DS-strategy"

# Agent Workspace: если существует — отчёты идут туда
AGENT_WORKSPACE="/home/igvel/repos/IWE/DS-agent-workspace"
...
STRATEGIST_LOG="/home/igvel/logs/strategist/$DATE.log"
...
"
        if echo "$warnings" | grep -q "push failed" 2>/dev/null; then
            report+="- **push failed:** Mac был оффлайн. Запусти \`cd /home/igvel/repos/IWE/DS-strategy && git pull --rebase && git push\`
"
        fi
    else
        report+="
