...
> 1. `/home/igvel/repos/IWE/FMT-exocortex-template/roles/extractor/config/routing.md` — таблицы маршрутизации (Pack'и, типы, директории)
> 2. `/home/igvel/repos/IWE/FMT-exocortex-template/roles/extractor/config/feedback-log.md` — лог отклонённых кандидатов (не предлагай аналогичные)
...
### Шаг 4e: Двойной routing — domain vs implementation

> Для каждого кандидата определи: domain или implementation?

1. **Тест доменности:** Используется ли это понятие/решение в другом проекте/контексте?
   - **Да (доменное)** → маршрутизируй в Pack (шаги 4a-4d)
   - **Нет (реализационное)** → маршрутизируй в DS docs/ (шаг 4f)

2. **Если сессия в downstream-репо**, дополнительно:
   - **Прочитай** `ontology.md` текущего downstream-репо (если существует)
   - Для доменных кандидатов → предложи обновить downstream ontology.md
   - Для реализационных → добавь привязку к Pack-понятию
   - Проверь: все ли существующие понятия downstream ontology.md актуальны

### Шаг 4f: Формализация реализационных кандидатов (DS docs/)

> Для кандидатов, прошедших тест как «реализационное» в шаге 4e.

1. **Определи целевой DS-репо** — используй таблицу «DS-репо по системам» из `config/routing.md` §5
2. **Определи тип:** сценарий → `docs/scenarios/`, процесс → `docs/processes/`, данные → `docs/data/`, архитектура → `implementation.md`
3. **Проверь:** существует ли уже файл? Если да → предложи обновить (не дублировать)
4. **Создай содержимое** по шаблону DS (без frontmatter trust/epistemic — это не Pack):
...
5. **В Extraction Report** реализационные кандидаты отображаются так же, как доменные, но с пометкой `→ DS docs/` вместо `→ Pack`
