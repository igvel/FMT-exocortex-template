...
Каждый репозиторий в `~/repos/IWE/` содержит `WORKPLAN.md` в корне с текущими РП.

**Агрегация:** При создании плана дня/недели Стратег:
1. Обходит все `~/repos/IWE/*/WORKPLAN.md`
...
```bash
for repo in $(ls ~/repos/IWE); do
  if [ -d ~/repos/IWE/$repo/.git ]; then
    echo "=== $repo ===" && cd ~/repos/IWE/$repo && git log --oneline --since="1 week ago" 2>/dev/null
  fi
done
```
...
> **Полное описание:** `~/repos/IWE/CLAUDE.md` секция 2.
