...
```
Если есть новые коммиты → добавить в «Требует внимания»: «[repo] обновлён upstream → `cd {{WORKSPACE_DIR}}/[repo] && git pull --rebase`». После pull FPF/SPF → reindex: `bash {{WORKSPACE_DIR}}/DS-MCP/knowledge-mcp/scripts/selective-reindex.sh FPF` (или SPF). Если DS-MCP отсутствует → пометить «требуется ручной reindex».
...
<details>
<summary><b>Разбор заметок (N)</b></summary>
