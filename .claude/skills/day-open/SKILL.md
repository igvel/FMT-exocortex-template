...
Коммиты за вчера по всем `{{HOME_DIR}}/IWE/*/` репо. Сопоставить с DayPlan.
...
**Проверка обновлений:** `cd {{HOME_DIR}}/IWE/FMT-exocortex-template && bash update.sh --check 2>&1`. Если доступно обновление → добавить в «Требует внимания»: «Доступно обновление IWE → `/iwe-update`».

**Проверка Base-репо (FPF, SPF, ZP):**
```bash
for repo in FPF SPF ZP; do
  dir="{{HOME_DIR}}/IWE/$repo"
