...
> **Три стадии, два масштаба, три протокола.** ОРЗ — фрактальный паттерн: День и Сессия.
...
| **День** | Закрытие | `protocol-close.md § День` | «закрываю день» / «итоги дня» |
...
| **Сессия** | Закрытие | `protocol-close.md § Сессия` | «закрываю» / «всё» / «закрывай» |
...
## 7. Кастомизация протоколов (AUTHOR-ONLY зоны)

Протоколы в `memory/protocol-*.md` содержат **платформенный контент** (от FMT-шаблона) и **пользовательский контент** (специфичный для конкретного IWE).

### Как добавить свои проверки / блоки

Оберни пользовательский контент маркерами:

```markdown
<!-- AUTHOR-ONLY: Краткое описание -->
Твой контент: проверки, скрипты, правила
<!-- /AUTHOR-ONLY -->
```

**Правила:**
- Пользовательский контент добавлять **рядом с релевантным платформенным** (не в конец файла)
- Маркеры — HTML-комментарии, невидимы при рендере
- При обновлении шаблона (template-sync) AUTHOR-ONLY зоны **вырезаются** и заменяются на `<!-- YOUR CUSTOM CHECKS HERE -->`
- Claude Code видит маркеры и **не затирает** пользовательский контент при обновлении платформенных блоков
- Если пользовательский блок **противоречит** платформенному — Claude Code обязан подсветить конфликт

### Плейсхолдеры `<!-- YOUR CUSTOM CHECKS HERE -->`

В шаблоне стоят плейсхолдеры — места, где пользователь может добавить свои блоки. Замени плейсхолдер на AUTHOR-ONLY зону с нужным контентом.
