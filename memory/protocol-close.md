...
- **Непривитые ветки:** `git log --all --oneline` vs `git log --oneline` — есть коммиты вне main? → «Требует внимания»
<!-- EXTENSION POINT: загрузить extensions/day-close.checks.md если существует -->
...
- [ ] **Непривитые ветки:** нет коммитов вне main (`git log --all` vs `git log` по репо с коммитами за день)
